...
# В реальном проекте здесь были бы вызовы внешнего API
class InternalBackend:
    def __init__(self):
        ...
        # Заполним слотами на неделю вперёд
        self._generate_slots()

    def _generate_slots(self):
        now = datetime.now().replace(minute=0, second=0, microsecond=0)
        start = now + timedelta(hours=2)  # чтобы были доступные заезды
        for day in range(7):
            ...
            for hour in [10, 12, 14, 16, 18]:
                ...
                # Оставляем несколько свободных
                available = min(total, max_novice) if max_novice else total
                if day == 0 and hour == 10:  # один слот почти полный
                    available = 1
    ...
    # Методы, имитирующие вызовы внутреннего бэкенда
    def get_slots(self, from_date: Optional[datetime] = None, to_date: Optional[datetime] = None):
        # Если даты не заданы – по умолчанию сегодня и +7 дней
        ...
        for slot in self.slots.values():
            if from_date <= slot.startTime < to_date:
                result.append(slot)
    ...
    def create_booking(self, user_id: str, slot_id: str, participants: int, need_rental: bool, comment: str = None):
        ...
        # Атомарно уменьшаем количество свободных мест (в реальности это делается в БД)
    ...
    def cancel_booking(self, booking_id: str, user_id: str):
        ...
        # Проверяем, можно ли отменить (не менее чем за 10 минут до старта)
...
def get_current_user(authorization: str = Header(...)):
    # Простейшая имитация: принимаем заголовок "Bearer user1" для demo
...
async def get_slots(from_date: Optional[str] = None, to_date: Optional[str] = None):
    """
    Получение списка слотов с фильтром по дате.
    По умолчанию – ближайшие 7 дней от текущего момента.
    """
    ...
    # Если даты не заданы – ставим сегодня и +7 дней
...
async def create_booking(booking_data: BookingCreate, user_id: str = Depends(get_current_user)):
    try:
        ...
        # Здесь можно отправить push-уведомление (заглушка)
        return booking
    except ValueError as e:
        # Для конфликтов (нет мест) возвращаем 409
